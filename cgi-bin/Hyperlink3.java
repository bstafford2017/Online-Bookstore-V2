        ...
        try {
            ...
            while(rset.next()){
                if(!list.contains(rset.getString(4))){
                    ...
                    String purchaseData = "SELECT isbn, title, price FROM book b WHERE isbn LIKE '" + rset.getString(4) + "'";
                    ResultSet result = innerstmt.executeQuery(purchaseData);
                    while(result.next()){
                        System.out.print("<br/><br/>ISBN: " + result.getString(1) + "<br/>Title: <a href=\"hyperlink.cgi?isbn=" + result.getString(1) + "\" style=\"color: black;\">" + result.getString(2) + "</a><br/>Price: $" + result.getString(3) + "<br/>Quantity: ");
                        CallableStatement cstmt = conn.prepareCall("{? = call quantity(?)}");
                        cstmt.registerOutParameter(1, Types.INTEGER);
                        cstmt.setString(2, result.getString(1));
                        cstmt.execute();
